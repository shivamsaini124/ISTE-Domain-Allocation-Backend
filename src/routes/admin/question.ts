...
import { mcqQuestionSchema, textQuestionSchema } from "../../validation/admin/question.js";
...
export const updateQuestionnaire = async (req: Request<{ questionnaireId: string }>, res: Response) => {
    try {
        ...
        // Just basic update for now (e.g. dueDate)
        // If we want to add/remove questions, that logic would be more complex
        const updated = await Questionnaire.findByIdAndUpdate(questionnaireId, req.body, { new: true }).lean();

        if (!updated) {
            return res.status(404).json({ message: "Questionnaire not found" });
        }

        res.status(200).json({ message: "Questionnaire updated successfully", data: updated });
    } catch (error) {
